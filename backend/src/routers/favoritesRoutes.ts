...
router.get("/:userId", async (req: Request<FavoritesParamsType>, res) => {
	try {
		...
		const products = await Favorite.findAll({ where: { userId: Number(userId) } });
		if (!products.length) {
			return res.status(404).json({
				message: "У данного пользователя еще нет избранных товаров",
			});
		}
...
router.delete("/:userId", async (req: Request<FavoritesParamsType>, res) => {
	try {
		...
		await Favorite.destroy({ where: { userId: Number(userId) } });
